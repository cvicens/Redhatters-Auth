    ...
    db.list(USERS_COLLECTION_NAME, filter, function (err, data) {
      if (err) {
        ...
        if (data && data.length == 1) {
          ...
          resolve({status: 200, message: 'Successfully Authenticated', sessionToken: 'dummyToken', roles: data[0].roles});
        } else {
          // Wrong credentials
          console.error('Credentials for username', username, 'ko');
          reject({status: 401, message: 'User unkonwn or wrong password'});
        }
      }
    });
  });
